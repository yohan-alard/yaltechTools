...
use crate::config;
use crate::models::{Invoice, SupplierInvoice};
...
pub async fn fetch_invoices(access_token: &str) -> anyhow::Result<Vec<Invoice>> {
    let resp = Client::new()
        ...
        .send()
        .await
        .context("Qonto client invoices réseau")?;

    if !resp.status().is_success() {
        let s = resp.status();
        return Err(anyhow!(
            "Qonto client invoices HTTP {}: {}",
            s,
...
pub async fn fetch_supplier_invoices(access_token: &str) -> anyhow::Result<Vec<SupplierInvoice>> {
    let resp = Client::new()
        ...
        .send()
        .await
        .context("Qonto supplier invoices réseau")?;
    ...
    Ok(resp
        ...
        .await
        .context("parse supplier invoices")?
        .supplier_invoices
        .into_iter()
        .map(|i| {
            let (amount_cents, currency) = i
                .total_amount
                .map(|a| {
