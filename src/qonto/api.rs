...
struct QontoSupplierInvoice {
    ...
    #[serde(default)] currency: String,
    #[serde(default)] due_date: Option<String>,
    #[serde(default)] supplier: Option<QontoParty>,
}
...
pub async fn fetch_supplier_invoices(access_token: &str) -> anyhow::Result<Vec<SupplierInvoice>> {
    ...
    Ok(resp.json::<SupplierListResponse>().await.context("parse supplier invoices")?
        .supplier_invoices.into_iter().map(|i| SupplierInvoice {
            label:    if i.label.is_empty() { i.supplier.as_ref().map(|s| s.name.clone()).unwrap_or_default() } else { i.label },
